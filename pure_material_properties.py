...
class PhaseData:
    ...
    """
    def __init__(self,
                 g0: Callable[[float], float],
                 v: Callable[[float], float],
                 surface_tension: Callable[[float], float]
                 ):
        self.g0 = g0
        self.v = v
    ...
    def __init__(self):
        ...
        self.phases = {
            'FCC' : PhaseData(
                ...
                surface_tension= lambda T: 1.675 - 0.47e-3*T
            ),
            'Liquid' : PhaseData(
                ...
                surface_tension= lambda T: 1e-3*(1207-0.228*T)
            )
        }
...
class NiData(BaseMaterial):
    def __init__(self):
        ...
        self.phases = {
            'FCC' : PhaseData(
                g0= lambda T: -5179.159 + 117.854 * T - 22.096 * T * log(T) - 4.8407e-3 * (
                            T ** 2)
                if T < 1728 else
                -27840.620 + 279.134977 * T - 43.1 * T * log(T) + 1127.54E28 * (T**-9),
                v= lambda T: 6.6e-6,
                surface_tension= lambda T: 2.94 - 3.92e-4*T
            ),
            'Liquid' : PhaseData(
                ...
                surface_tension= lambda T: 2.488 - 0.393e-3*T
            )
        }
        self.atomic_radius = 163e-12
    ...
    def __init__(self):
        ...
        self.phases = {
            'FCC' : PhaseData(
                ...
                surface_tension= lambda T: 2.158512 - 0.4e-3*T
            ),
            'Liquid' : PhaseData(
                ...
                surface_tension= lambda T: 1e-3*(1585-0.211*T)
            )
        }
